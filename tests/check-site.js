...
function walk(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    if (entry.name === 'tests' || entry.name === '.git' || entry.name === '.superpowers' || entry.name === '.worktrees' || entry.name === 'node_modules' || entry.name === 'google28003a8fb6bb282a.html') continue;
